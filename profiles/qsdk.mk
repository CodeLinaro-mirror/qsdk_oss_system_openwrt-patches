...
WIFI_PKGS:=kmod-qca-wifi-unified-profile \
	qca-hostap qca-hostapd-cli qca-hapd-supp qca-hapd-supp-macsec qca-wpa-supplicant \
...
define Profile/QSDK_Premium
	...
	PACKAGES:=$(OPENWRT_STANDARD) $(STORAGE) $(AUDIO) \
		...
		$(QCA_EZMESH) $(OPENVPN) kmod-macvlan kmod-qca-hyfi-bridge kmod-netstandby-eawtp-drv \
...
define Profile/QSDK_Open
	...
	PACKAGES:=$(OPENWRT_STANDARD) $(STORAGE) $(TEST_TOOLS) $(AUDIO) $(VIDEO) \
		...
		$(WIFI_OPEN_PKGS) $(USB_ETHERNET) $(NSS_COMMON) $(SAL_QOS) $(NSS_STANDARD) $(NSS_MESH) \
		$(QCA_ECM_PREMIUM) $(MAP_PKGS) $(IGMPSNOOPING_RSTP) $(IPSEC) $(QOS) -lacpd  \
		$(STRONGSWAN) $(NSS_PPE) $(NSS_USERSPACE_OSS) $(NSS_NSM) $(AQ_PHY) $(MACSEC_OPEN_PKGS) \
		...
		kmod-macvlan myftm $(NSS_FLOWID) $(NSS_FLS) kmod-qca-nss-ppe-ds
endef
...
define Profile/QSDK_512
	...
	PACKAGES:=$(OPENWRT_STANDARD) $(AUDIO) $(NSS_COMMON) $(NSS_STANDARD) $(SWITCH_SSDK_PKGS) \
		$(WIFI_PKGS) $(WIFI_FW_PKGS) $(STORAGE) $(CD_ROUTER) kmod-netstandby-eawtp-drv \
...
define Profile/QSDK_16M
	...
	PACKAGES:=wififw_mount_script $(NSS_COMMON) $(NSS_PPE_16M) $(SWITCH_SSDK_PKGS) \
		...
		-kmod-bt_tty -kmod-clk-test -sysupgrade-helper kmod-netstandby-eawtp-drv \
